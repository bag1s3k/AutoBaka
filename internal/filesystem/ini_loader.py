...
class IniConfigFile:
    ...
    def __init__(self, ini_path: Path):
        ...
        """
        logger.debug("Create an object that represents config of the app")
    ...
    def set_new(self, section: str, option: str, value: str) -> bool:
        try:
            ...
            if (
                    not log_variable(self.config.has_section(section), "critical", f"No section named {section}", f"Section {section} found")
                    or
                    not log_variable(self.config.has_option(section, option), "critical", f"No option named {option}", f"Option {option} found")
            ):
                return False
    ...
    def _read_config(self) -> bool:
        ...
        try:
            if not log_variable(self.ini_path.exists(), "critical", f"Configuration file doesn't exist, path: {self.ini_path}", f"Configuration file found, path: {self.ini_path}"): return False
    ...
    def get_config(self, section: str, option: str):
        ...
        if (
                not log_variable(self.config.has_section(section), "critical",
                                f"No section named {section}", f"Section {section} found")
                or
                not log_variable(self.config.has_option(section, option),
